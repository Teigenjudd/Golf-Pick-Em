...
|---|---|---|---|
| `01-login.dc.html` | **Login** | Any | Centered single column, max-w 360px. Wordmark above card. Card contains form or success state. Demo link below card. Two conditional states (`showForm` / `showSent`). **As-built adds a third axis not in this prototype:** a "Sign-in link" / "Password" pill toggle above the form (link tab default, styled like the toggle described under "State Toggle (Login / Join Prototype)" below) — see `docs/PAGES.md` §1. |

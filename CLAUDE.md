...
- **Sport #2 = CFB (planning done, no code yet):** college football is the committed next sport — weekly against-the-spread, season-cumulative, fully siloed from golf (own `cfb` schema, `lib/cfb.js`, scoring engine, picks UI, `cfd-proxy`; only the `pool_standings` output shape is shared). Rules of record: `docs/CFB_FORMAT.md`. PR-sliced build plan: `docs/CFB_BUILD_PLAN.md` (PR0 planning shipped 2026-08-11; PR1, the `cfb` schema scaffold, has not started).

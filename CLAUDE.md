...
- **Sport #2 = CFB (in build):** college football, weekly against-the-spread + season-cumulative, fully siloed from golf — its own `cfb` Postgres schema (`event_details`, `weeks`, `games`, `picks`), its own `src/lib/cfb.js` seam (the only file calling `supabase.schema('cfb')`), its own scoring engine (`src/utils/cfbScoring.js`, mirrored server-side at `supabase/functions/_shared/cfbScoring.ts` — shared fixtures keep the two in parity), and its own data provider (CollegeFootballData). Only the shared `public.pool_standings` `{user_id, rank, total, display}` output shape is common with golf. **What exists today:** the full backend (schema + RLS + `cfb.cfb_submit_week_picks`, the whole-card submit RPC that is the *only* write path onto `cfb.picks`); the grader (`grade-cfb-week` edge fn, the first writer of `public.pool_standings`); three CFB pollers (`poll-cfb-lines` hourly for slate/spreads, `poll-cfb-scores` for live in-game scores — both talk to CFBD directly; `cfd-proxy` is the admin-JWT proxy); admin pool-creation + weekly-ops pages (`/admin/cfb*`); and the player Pool Detail / leaderboard page (`/cfb/pool/:id`, Varsity Navy theme in `src/theme/cfb.js`). CFB uses **per-pool events** like golf; graders and pollers dedupe CFBD calls by real `(season, week_number)`. Spreads are the CFBD lines pulled automatically — CFB has no admin "odds" step. **Not built yet:** the weekly picks builder (`/cfb/pool/:id/picks` is still a placeholder) and the sport-dispatch layer, so a CFB pool is reachable only by a direct admin link, not the join-code flow. Rules of record: `docs/CFB_FORMAT.md`. Build sequence + shipped history: `docs/CFB_BUILD_PLAN.md`, `docs/CFB_UI_PLAN.md`, and the merged PRs.

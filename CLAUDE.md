...
**Theme system:** Two registers — *general* (auth, dashboard, admin — brand-level, sport-agnostic) and *sport-specific* (pool detail + picks page — theme swaps per sport type). Golf is fairway green/gold. CFB's register — "Varsity Navy" (navy `#101C3D→#0A1229` header gradient, brick `#D6291B` accent, green `#2E8F4F` cover/win — constants in `src/theme/cfb.js`) — is locked and applied to two placeholder pages (`/cfb/pool/:id`, `/cfb/pool/:id/picks`, shipped 2026-08-13, PR #48); real content on those pages is still being built. Future sports get their own theme applied only on those two pages.

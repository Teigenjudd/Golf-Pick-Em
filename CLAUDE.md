...
- **Sport #2 = CFB (data layer, scoring engine, and grading shipped through PR5, no picks UI yet):** college football is the committed next sport — weekly against-the-spread, season-cumulative, fully siloed from golf (own `cfb` schema, `lib/cfb.js`, scoring engine, picks UI, `cfd-proxy`; only the `pool_standings` output shape is shared). Rules of record: `docs/CFB_FORMAT.md`. PR-sliced build plan: `docs/CFB_BUILD_PLAN.md` (PR0 planning shipped 2026-08-11; **PR1 shipped 2026-08-11, PR #40** — additive migration `supabase/migrations/20260811000000_cfb_phase1_scaffold.sql` creates the empty `cfb` schema (`event_details`, `weeks`, `games`, `picks`), seeds the `public.sports` row for `'cfb'`, grants `authenticated`/`service_role`, and enables RLS with no policies yet (deny-all). **PR2 shipped 2026-08-11, PR #41** — migration `supabase/migrations/20260811000001_cfb_phase2_rls_and_submit_rpc.sql` adds RLS policies (reference tables: authenticated-read + admin-manage; `cfb.picks`: read-own always, read others' confirmed picks only after that pick's week locks) and `cfb.cfb_submit_week_picks(p_pool_id, p_week_id, p_picks)` — a `SECURITY DEFINER` function that is the **only** write path onto `cfb.picks` (no client insert/update/delete policy exists; a per-row policy can't enforce the whole-card rule, so none was added even for defense-in-depth). It validates membership, week-belongs-to-pool's-season, week-still-open (gated on `lock_time`, not the `status` label — a week is pickable once its slate exists and lock_time hasn't passed), and the full 6-pick card (5 ATS + 1 underdog on 6 distinct games, ≤1 double-down on an ATS pick, valid teams), then atomically replaces the card and freezes `locked_spread` server-side. **PR3 shipped 2026-08-12** — the CFBD data layer: migration `supabase/migrations/20260812000000_cfb_phase3_slate_import_support.sql` adds `public.api_usage.cfbd_calls` (mirrors `slash_golf_calls`) and a `CHECK (underdog_spread IS NULL OR > 0)` on `cfb.games`; `supabase/functions/cfd-proxy/index.ts` is an admin-JWT gated CollegeFootballData proxy (`CFBD_API_KEY` Supabase secret, endpoint allowlist `games`/`lines`/`teams/fbs`, 1000/mo cap — CFBD's free tier confirmed at 1000/month, not golf's 1800 placeholder); `src/lib/cfbd.js` is the browser proxy client; `src/lib/cfb.js` is the `cfb` schema seam (only file calling `supabase.schema('cfb')`) — pure `buildGameRows()` filters CFBD's games+lines to FBS-vs-FBS games with a posted line, freezes `home_spread` signed from home's perspective, and stores `underdog_team`/`underdog_spread` positive+non-NULL, the contract PR2's RPC trusts verbatim; `importWeekSlate()` fetches via the proxy and upserts on `cfbd_game_id`. Senior review (`agents/senior-dev/reviews/cfb-pr3-slate-import.md`, APPROVE WITH QUESTIONS) prompted two founder-decided safety additions, both in-branch (`agents/pm/DECISIONS.md`, 2026-08-12): a sign cross-check (`chooseLine()`/`favoriteFromFormattedSpread()` compare CFBD's numeric spread sign against its own `formattedSpread` text label, e.g. `"Michigan -17.5"`; on disagreement the game is skipped and a warning logged rather than freezing a wrong underdog — CFB has one data provider, unlike golf's two-source corroboration) and a week guard (`importWeekSlate` asserts the target `cfb.weeks` row's `week_number`/season match the CFBD week being pulled, else throws). Verified against real 2025 Week 1 data (48 of 96 games eligible, sign mapping correct on every row) plus 25 transform fixtures. All three migrations are already applied to prod via `supabase db push` (no users yet, so prod doubles as the CFB dev DB), but `cfb` stays hidden from the Data API until the prod dashboard's Exposed Schemas toggle is flipped — a deferred cutover step; admin writes via `.schema('cfb')` work regardless, since that's RLS + grants, not schema exposure. No admin UI yet (PR9). Nothing golf reads is touched. **PR4 shipped 2026-08-12, PR #43** — `src/utils/cfbScoring.js`, a pure import-free scoring engine implementing `docs/CFB_FORMAT.md` verbatim (`doubleDownBuffer`, `doubleDownWinBy`, `gradeAtsPick`, `gradeDoubleDown`, `underdogTier`, `gradeUnderdogPick`, `gradeWeekCard`, `projectSeasonStandings` — the cumulative fold into the shared `public.pool_standings` `{user_id, rank, total, display}` shape), plus the repo's first unit tests (`src/utils/cfbScoring.test.js`, 44 passing, `vitest` + a `test`/`test:watch` script added to `package.json` — starts closing BACKLOG F4). No app code imports the engine yet (consumed by `grade-cfb-week` in PR5 and the picks UI in PR6). Senior review (`agents/senior-dev/reviews/cfb-pr4-scoring-engine.md`, APPROVE WITH QUESTIONS) confirmed the math is sound and prompted two founder decisions (`agents/pm/DECISIONS.md`, 2026-08-12): PR5's TS grading mirror will share test fixtures with the JS suite so drift fails a test, not just a comment; and a double-down is legal on an underdog ATS pick, so `doubleDownWinBy` returns a sign-general margin threshold and PR6's copy must phrase the bonus condition generally ("cover by more than N") rather than favorite-only "win by X+". **PR5 shipped 2026-08-12, PR #44** — the authoritative grader: `supabase/functions/grade-cfb-week/index.ts`, a service-role edge function gated cron-secret-or-admin-JWT (golf's `poll-leaderboard` pattern), grading one targeted week (`{week_id}`) or scanning every week whose `lock_time` has passed and isn't yet `graded`. Dedupes CFBD's `/games` by real `(season, week_number)` — one fetch fanned to every event's games sharing it (golf's D3 dedup pattern), counted against `public.api_usage.cfbd_calls`. Writes final scores to `cfb.games`, grades every pick via `supabase/functions/_shared/cfbScoring.ts` (a hand-kept TS mirror of `src/utils/cfbScoring.js`, since Deno can't import `src/`), sets week status `graded`/`locked`, and recomputes each affected pool's season-cumulative `public.pool_standings` via `projectSeasonStandings` — the first code in the repo to write that table (closes the CFB half of BACKLOG F1). Only completed games grade; re-running is idempotent. The PR4-promised drift guard shipped with it: `src/utils/cfbScoring.fixtures.js` holds shared worked-example cases run against both the JS engine (`cfbScoring.test.js`) and the TS mirror (`cfbScoring.parity.test.js`), asserting identical output (152 tests pass — the repo's first server/client parity suite). Also added: `effectiveDoubleDownLine(lockedSpread)` (both engine + mirror) — the buffer-adjusted double-down line in the picked team's sign convention, for PR6's picks UI. `src/lib/cfb.js` adds `gradeCfbWeek(weekId)`, a thin invoker for PR9's admin "Grade week" button; grading itself never runs client-side. No migration (all tables exist; service role bypasses RLS). Senior review (`agents/senior-dev/reviews/cfb-pr5-grading.md`, APPROVE WITH QUESTIONS) found the happy path correct with no blockers and prompted two founder decisions, both deferred to PR9 (`agents/pm/DECISIONS.md`, 2026-08-12): an admin "finalize week as-is" override for a week stuck on a cancelled/rescheduled game (otherwise it re-polls CFBD every cron run forever), and a `lock_time` guard on the manual `{week_id}` grade path (today ungated, unlike the scan path). No cron or admin UI calls this function yet (PR9); no app screen reads `pool_standings` yet (PR7). Picks UI is still PR6+, not built).

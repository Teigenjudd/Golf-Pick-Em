...
| 3.4 | **Second sport — CFB, code-complete (PR0 shipped 2026-08-11)** — college football, weekly against-the-spread/season-cumulative, full schema/UI/data-provider siloing (`docs/CFB_FORMAT.md`, `docs/CFB_BUILD_PLAN.md`, ~11-PR sequence) | Chosen deliberately ahead of golf being fully "proven" — supersedes this row's earlier NFL placeholder; see DECISIONS 2026-08-11. Every player and admin screen is now built — data layer, scoring/grading, live scores, admin (including a sport-agnostic create chooser + Golf|CFB switcher), dashboard/join reachability, the weekly picks builder, its read-only locked/auto-filled/graded card (PR-B, shipped 2026-08-14), and automatic missed-deadline handling (PR10, shipped 2026-08-14) — every core build PR is now shipped — see status-log rows above. Still open: the prod cutover of the three billable CFBD pollers (deploy edge functions, arm cron — PR9); the free auto-fill/lock cron is already armed and running independently of that cutover. | High (later) | 🟡 |

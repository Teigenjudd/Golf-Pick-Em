# Poold — Decision Log (Archive)

> **What this file is:** older entries from `agents/pm/DECISIONS.md`, split out to keep the
> live log small. Everything here is still authoritative and still searched by grep — it's
> just history that no longer needs to sit in the file the PM opens each merge.
>
> **Do not append here.** New decisions always go to the top of `agents/pm/DECISIONS.md`.
> This archive holds decisions from **2026-08-10 and earlier**. Newest first.

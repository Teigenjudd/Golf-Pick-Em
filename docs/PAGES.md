...
**What it does:** Account settings — reached from the header avatar on Dashboard (there is no bottom nav; see Shared Components). Lets an existing user change their display name; also the sign-out surface. Deliberately shows **no email address** — see the note under Dashboard.
...
**What must be on this page:**
- App wordmark; header "+" button (opens the Join sheet for a player, the Add-a-pool chooser sheet for an admin) and `ProfileMenu` avatar
...
- Inline error line if the role change is refused
...
**Editing an existing card resets it, doesn't pre-fill it:** the Dashboard tile's
"Edit picks" button confirms first ("All your existing picks for this week will be
reset...") then navigates here with `?reset=1`, which makes the builder start **empty**
instead of pre-filling the saved card. Rationale: `cfb_submit_week_picks` re-freezes
`locked_spread` from the *current* game row for every pick in the resubmitted payload,
not just the changed ones — silently pre-filling old picks could look unchanged on
screen while actually re-locking them to a line that's since moved. The saved card in
the DB is untouched until a full valid card is resubmitted (`reset=1` only clears local
builder state). Direct navigation without the param still pre-fills, for a first-time
visit to an in-progress card.
...
- The (filtered/sorted) slate as a list of `CfbGameCard`s: two ATS team chips (tap to
  ...
  versa; once 5 ATS slots are full, other games' ATS chips disable.
...
locks.") · open-reset (`?reset=1` — builder starts empty, "Your previous picks were
reset — build a fresh card from today's lines." banner instead of the editing notice) ·
